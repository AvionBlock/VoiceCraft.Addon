export class UTF8 {
   ...
   */
  public static GetBytes(s: string): Uint8Array | undefined {
  ...
  public static GetString(bytes: Uint8Array, index: number, count: number): string {
    ...
    while (index < maxIndex) {
      ...
      if (charCode !== undefined) {
        ...
        index += this.GetByteCountFromCharCode(charCode) as number; //We know it's a valid utf8 character.
        continue;
      }
   ...
   */
  public static GetByteCount(chars: string): number {
    let bytesCount = 0;
  ...
  public static GetCharCount(bytes: Uint8Array, index: number, count: number): number {
    ...
    let charCount = 0;
    while (index < maxIndex) {
      if ((bytes[index] & 0xc0) !== 0x80) charCount++;
      index++;
    }
   ...
   */
  public static GetMaxByteCount(charCount: number): number {
   ...
   */
  public static GetByteCountFromCharCode(charCode: number): number | undefined {
    if (charCode <= 0x7f) {
      return 1;
   ...
   */
  public static GetBytesFromCharCode(charCode: number): Uint8Array | undefined {
