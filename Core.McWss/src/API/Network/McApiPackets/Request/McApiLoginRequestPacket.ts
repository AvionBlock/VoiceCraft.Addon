import {McApiEventType, McApiPacketType} from "../../../Data/Enums";
...
export class McApiLoginRequestPacket implements IMcApiPacket, IMcApiRIdPacket {
  constructor(requestId: string = "", token: string = "", version?: Version, subscribeEvents?: McApiEventType[]) {
  ...
  public get SubscribeEvents(): McApiEventType[] {
  ...
  private _subscribeEvents: McApiEventType[];
  ...
  public Serialize(writer: NetDataWriter) {
    ...
    for (let event of this._subscribeEvents) {
  ...
  public Deserialize(reader: NetDataReader) {
    ...
    for (let i = 0; i < eventsLength; i++) {
      const eventType = reader.GetInt();
      if (eventType in McApiEventType) {
        events.push(eventType as McApiEventType);
      }
    }
    this._subscribeEvents = events;
  }

  public Set(requestId: string = "", token: string = "", version?: Version, subscribeEvents?: McApiEventType[]): McApiLoginRequestPacket {
    this._requestId = requestId;
    this._token = token;
    this._version = version ?? new Version(0, 0, 0);
    this._subscribeEvents = subscribeEvents ?? [];
